...
// Import Firestore helper to fetch restaurant reviews
...
// Async function that generates a Gemini-based summary of restaurant reviews
export async function GeminiSummary({ restaurantId }) {
  // Get a Firebase app instance authenticated for the current user
  const { firebaseServerApp } = await getAuthenticatedAppForUser();

  // Fetch all reviews for the given restaurant ID from Firestore
  ...
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      ...
      throw new Errpr( // <-- Typo: should be "Error"
